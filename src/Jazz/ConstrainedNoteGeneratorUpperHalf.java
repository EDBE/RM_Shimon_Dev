...
public class ConstrainedNoteGeneratorUpperHalf extends PitchGenerator{
	...
	public ConstrainedNoteGeneratorUpperHalf(Physicality constraints){
		super();
	...
	private List<List<Integer>> viterbiMelody(List<List<Integer>> melody, int moveTimeConstraint){		
		...
		int index;
		noteSize = pConstraints.noteSize;
		for(int note=pConstraints.lowestNote;note<noteSize;note++){
			//int note = possibleNotes.get(j);
			index = note;//-pConstraints.lowestNote;
			musicScore = melodyEmissionScore(note,melody.get(0).get(0));
			for(int i=0;i<pConstraints.configurations.get(index).size();i++){
				//physicalityScore = 1;
				physicalityScore = pConstraints.transitionScores[pConstraints.configurations.get(index).get(i)][startState];
				state = pConstraints.stateDict[index][i];
